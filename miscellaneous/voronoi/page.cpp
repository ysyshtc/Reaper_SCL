...
inline point* Other(const Edge *e, const point *p) { return e->Org == p ? e->Dest : e->Org; }
inline Edge* Next(const Edge *e, const point *p) { return e->Org == p ? e->Onext : e->Dnext; }
inline Edge* Prev(const Edge *e, const point *p) { return e->Org == p ? e->Oprev : e->Dprev; }
struct gEdge {
	int u, v;
	double w;
	gEdge() {}
...
inline void Low_tangent(Edge *e_l, point *o_l, Edge *e_r, point *o_r, Edge **l_low, point **OL, Edge **r_low, point **OR) {
